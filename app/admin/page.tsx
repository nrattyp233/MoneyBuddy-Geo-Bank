...
export default function AdminPage() {
  const isPaypalConfigured = process.env.PAYPAL_CLIENT_ID;
  const isDatabaseConfigured = process.env.DATABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL;
  const isGeminiAiConfigured = process.env.GOOGLE_GENERATIVE_AI_API_KEY;

  const integrationStatus = [
    {
      name: "PayPal Payments",
      status: isPaypalConfigured ? "configured" : "pending",
      description: "PayPal payment processing",
      icon: CreditCard,
    },
    {
      name: "Database",
      status: isDatabaseConfigured ? "configured" : "pending",
      description: isDatabaseConfigured
        ? "User data and transactions"
        : "Provide either DATABASE_URL or NEXT_PUBLIC_SUPABASE_URL in your environment configuration to establish a database connection.",
      icon: Database,
    },
    {
      name: "Gemini AI",
      status: isGeminiAiConfigured ? "configured" : "pending",
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {isPaypalConfigured ? 'Configured' : 'Not configured'}
                  </p>
                </div>
              </div>
              <Badge variant={isPaypalConfigured ? "default" : "secondary"}>
                {isPaypalConfigured ? 'Active' : 'Pending'}
              </Badge>
            </div>
          </CardContent>
        </Card>
